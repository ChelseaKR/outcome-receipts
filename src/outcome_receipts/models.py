...
class Receipt:
    ...
    committed eval is reproducible. ``unit`` is the metric's unit (``count``,
    ``percent``, ``money``, ``duration``, or ``rate``), carried so a consumer can
    re-derive the display. ``definition`` carries the figure's plain-language
