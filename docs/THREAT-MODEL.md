...
| One partner's rows are submitted twice and summed under a `disjoint` rollup declaration | Rollup rejects two partner receipts sharing a non-empty slice hash; the empty slice is exempt and a `not_deduplicated` plan keeps its label | Medium because partial overlap between different row sets stays an operator declaration |
