...
| R8 | **Human approval / sign-off step** before export (CLI confirm or a signed approval line in the manifest naming the approver). | P3,P4 | P2 | M | **[corroborates ROADMAP]** (the architecture names `approve` between suppress and export; no surface yet). ✅ Implemented 2026-07-02 (`run --approved-by` / interactive sign-off; approver recorded in the manifest and report, fail-closed off a TTY) |
...
| R10 | **Period-comparison legibility** — label direction and magnitude in the trace view, and bind the change figure's receipt to both period receipts. | P4,P8 | P2 | S | Extends the shipped `[comparison]` feature; the change is already one SQL query. **[NET-NEW]** ✅ Implemented 2026-07-02 |
...
| E2 | **Common-report / multi-funder reuse export** — generate the same receipted figure set into more than one funder template format. | P1,P3 | P1 | M | EV2: only ~1/3 of funders accept a common report and reporting outsources burden; reuse is the time win. Parallels the v1.0 "second template format." **[corroborates ROADMAP v1.0]** (partial) · **[NET-NEW]** framing ✅ Implemented (`[[report.templates]]` renders one shared figure set into several funder formats, each held to the same gate and written to its own `out/<id>/` subdir; see `examples/multi-funder`) |
| E3 | **Logic-model / indicator mapping** — optional `indicator` and `data_source` fields per metric tying a figure to a theory-of-change row, with collection frequency. | P8,C-group | P2 | M | EV4. Makes the tool legible to evaluators without becoming a measurement product. **[NET-NEW]** ✅ Implemented 2026-07-02 |
| E4 | **Metric-mapping agent** — map a funder template's required metrics to `MetricSpec`s over a schema-variant export (HMIS CSV and common funder shapes), with a review queue for low-confidence mappings. | P5 | P2 | L | **[corroborates ROADMAP v0.4]** ✅ Implemented as deterministic `receipts map`: documented HMIS aliases produce pending candidates; missing/ambiguous fields block; every mapping requires human review and none is executed. |
| E5 | **Policy-gated drafting seam** — optional Claude-on-Bedrock narrative drafter writing prose around receipted figures, guarded by the same gate; off by default. | P1,E1 | P2 | L | **[corroborates ROADMAP v0.3]** ✅ Implemented: config and per-run cloud opt-in, display-only prompt, raw and publishable grounding gates, human approval, and provider provenance. No judge ships; calibration remains mandatory if one is added. |
| E6 | **Tamper-evident, signed audit bundle** — package report, receipts, slice hashes, and a `verify` manifest as a signed artifact. | P10,P9 | P3 | M | Builds on R6 and the toward-1.0 supply-chain hardening (SBOM, Sigstore). EV5. **[corroborates ROADMAP]** (toward 1.0) ✅ Implemented 2026-07-02: stdlib self-contained keyed-BLAKE2b bundle (`bundle.py`, `run` writes `bundle.json`, `receipts verify-bundle`); Sigstore/OIDC keyless signing deferred to the release workflow (ADR 0004) |
| E7 | **Board / financial reconciliation view** — place receipted outcome figures next to the relevant financial lines and a cross-period change log. | P4,P3 | P3 | M | Extends the shipped board-report template. **[NET-NEW]** ✅ Implemented 2026-07-02 |
| E8 | **Model and data cards** for the drafting seam, regenerated on release. | P11,E1 | P3 | S | **[corroborates ROADMAP v0.3]** ✅ Implemented with generated `MODEL-CARD.md` / `DATA-CARD.md` and a tagged-release drift check. |
| E9 | **EN/ES report-output parity** — externalize report copy so the same receipted figures render bilingually. | P7,(LEP communities) | P3 | M | [`CLAUDE.md`](../CLAUDE.md) i18n bar (EN/ES parity); not in [`docs/ROADMAP.md`](ROADMAP.md). **[NET-NEW]** vs roadmap ✅ Implemented 2026-07-02 (`copy.py` catalog; `--locale en/es`; figures/SQL/hashes byte-identical across locales). **Prerequisite FIX-13 (locale-aware number canonicalization in the gate) — DONE:** the gate now binds a figure display and a prose span that denote the same value across locale thousands/decimal separators (US `1,234`, European `1.234`, NBSP-grouped `1 234`), so localized output cannot desync a number from its receipt. Written-out numerals stay unbound (fail-closed). See `src/outcome_receipts/grounding.py`, `tests/test_grounding_locale.py`. |
| E10 | **Data-flow map + retention model** documented in the DPIA. | P11 | P3 | S | Closes the explicit TODO in [`RESPONSIBLE-TECH-AUDITS.md`](RESPONSIBLE-TECH-AUDITS.md). EV6. **[NET-NEW]** (doc, not code) ✅ Implemented 2026-07-02 |
